...
void main() {
  ...
  test('parseAuthorizationCredentialAppleIDFromDeeplink: With User', () async {
    ...
    expect(parsed.givenName, 'Timm');

    expect(parsed.familyName, 'Preetz');

    expect(parsed.email, '4rtppgbhgb@privaterelay.appleid.com');
  });

  test('parseAuthorizationCredentialAppleIDFromDeeplink: With user but only email', () async {
    final deeplink = Uri.parse(
